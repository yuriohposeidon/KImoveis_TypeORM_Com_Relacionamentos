...
describe("POST /schedules", () => {
  ...
  beforeAll(async () => {
    await AppDataSource.initialize()
      .then(async (res) => {
        connection = res;
        [realEstate1, realEstate2] = await readRealEstateRouteMock.manyRealStations(2);

        userAdmin = await connection.getRepository(User).save(createUserRouteMock.userComplete);
  ...
  it("Error: Must not be able to create a schedule - Admin token - User schedule already exists", async () => {
    const payload = createScheduleRouteMock.schedulesSameDateDifferentRealEstate;
