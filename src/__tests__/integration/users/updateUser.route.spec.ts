...
describe("PATCH /users", () => {
  ...
  it("Success: Admin must be able to update a user - Admin token - Full body", async () => {
    ...
    expect(response.body).toEqual(expect.objectContaining({ ...payload, id: userNotAdmin.id }));
  ...
  it("Success: Admin must be able to self update - Admin token - Full body", async () => {
    ...
    expect(response.status).toBe(expectResults.status);
    expect(response.body).toEqual(expect.objectContaining({ ...payload, id: userAdmin.id }));
    expect(response.body).not.toEqual(expect.objectContaining({ password: expect.any(String) }));
  });
  ...
  it("Success: User must be able to self update - User token - Full body", async () => {
    const response = await supertest(app)
      .patch(updateUserUrl)
      .set("Authorization", `Bearer ${tokenMock.genToken(userNotAdmin.admin, userNotAdmin.id)}`)
      .send(updateUserRouteMock.userComplete);
  ...
  it("Success: User must be able to self update - User token - Partial", async () => {
    const response = await supertest(app)
      .patch(updateUserUrl)
      .set("Authorization", `Bearer ${tokenMock.genToken(userAdmin.admin, userNotAdmin.id)}`)
      .send(updateUserRouteMock.userPartial);
